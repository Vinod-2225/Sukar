...
import com.designmaster.sukar.adapters.SelectAreaAdapter
...
class SelectAreaDialogFragment: BottomSheetDialogFragment() {

    companion object {
        fun getInstance(zoneData: ArrayList<AreaInfo>): SelectAreaDialogFragment {
            val args = Bundle()
            args.putParcelableArrayList(AppConstants.INTENT_KEY.DATA, zoneData)
            val fragment = SelectAreaDialogFragment()
    ...
    private lateinit var governorateListAdapter: SelectAreaAdapter
    private var itemData = ArrayList<AreaInfo>()
    ...
    private fun setReasonList() {
        governorateListAdapter = SelectAreaAdapter(mContext, itemData) {
