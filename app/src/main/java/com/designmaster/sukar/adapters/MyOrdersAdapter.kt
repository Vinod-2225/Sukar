...
class MyOrdersAdapter(mActivity: Activity?, mList: ArrayList<MyOrdersInfo>) : RecyclerView.Adapter<MyOrdersAdapter.ViewHolder>() {
    private val mDataset: ArrayList<String>? = null
    var mContext: Context? = mActivity
    var mList: ArrayList<MyOrdersInfo> = mList
    ...
    inner class ViewHolder(itemView: View) : RecyclerView.ViewHolder(itemView), View.OnClickListener {
        ...
        var lblOrderId: TextView = itemView.findViewById<View>(R.id.lblOrderId) as TextView
        var lblStatus: TextView = itemView.findViewById<View>(R.id.lblStatus) as TextView
        var lblDate: TextView = itemView.findViewById<View>(R.id.lblDate) as TextView
        var lblDetails: TextView = itemView.findViewById<View>(R.id.lblDetails) as TextView
...
//        val v: View = LayoutInflater.from(parent.context).inflate(R.layout.bag_item_layout, parent, false)
        ...
        if (AppPrefs.isLocaleEnglish(mContext!!)) {
            v = LayoutInflater.from(parent.context).inflate(R.layout.my_orders_item, parent, false)
        }else{
            v = LayoutInflater.from(parent.context).inflate(R.layout.my_orders_item, parent, false)
        }
...
//        val transformation: Transformation = RoundedCornersTransformation(radius, margin)
        ...
        holder1.lblOrderId.text = mList[position].OrderID
        holder1.lblStatus.text = mList[position].orderStatus
        holder1.lblDate.text = mList[position].date



        holder1.lblDetails.setOnClickListener {
            clickListener.onDetailsClick(position)
        }
